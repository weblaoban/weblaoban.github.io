...
// donghuashijin  animate中的动画时间
	function tongshi(tuBox,tu,daBox,zuojian,youjian,tongshizhangshu){
			const box=$(tuBox)[0];
			const lis=$(tu);
			const win=$(daBox)[0];
			const zuobtn=$(zuojian)[0];
			const youbtn=$(youjian)[0];
			const lisW=parseInt(window.getComputedStyle(win,null).width);
			...
			function move(type="l"){
				flag=false;
				if(type=="r"){
					animate(box,{marginLeft:-lisW},500,function(){
						for(let i=0;i<n;i++){
					...
					box.style.marginLeft=-lisW+"px";
					animate(box,{marginLeft:0},500,function(){
					flag=true;
					});
