...
// donghuashijin  animate中的动画时间
	function tongshi(tuBox,tu,daBox,zuojian,youjian,tongshizhangshu,lunboshijian,donghuashijin){
			...
			const winW=parseInt(window.getComputedStyle(win,null).width);
			...
			function move(type="r"){
				flag=false;
				if(type=="r"){
					animate(box,{marginLeft:-winW},donghuashijin,function(){
						for(let i=0;i<n;i++){
			...
			zuobtn.onmouseover=function(){
				clearInterval(t);
			}
			zuobtn.onmouseout=function(){
				t=setInterval(move,lunboshijian);
			}
			youbtn.onmouseover=function(){
				clearInterval(t);
			}
			youbtn.onmouseout=function(){
				t=setInterval(move,lunboshijian);
			}
			youbtn.onclick=function(){
				if(flag){
					youbtn.style.color="#e0e0e0";
					move("r");
				}
			}
			zuobtn.onclick=function(){
				if(flag){
					zuobtn.style.color="#e0e0e0";
					move("l");
				}
			}
