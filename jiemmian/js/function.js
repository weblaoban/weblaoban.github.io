	...
	//11、楼层跳转 按需加载
	//12、鼠标滚轮事件
	//13、获取obj元素的style（width、height等）样式
	//14、cookie的添加、删除、获取
	//15、去空格
	//16、驼峰命名法
	//17、cookie的设置、获取删除
	...
	function jiedianlunbo(imgBox,zuojian,youjian,bigBox,bannerTu,num,lunboshijian,donghuashijian){
		...
		function move(type="r"){
			flag=false;
			if(type=="r"){
				let sec=box.children[2];
				let chi=box.children;
				for(let k=0;k<chi.length;k++){
					chi[k].classList.remove("xqac");
				}
				sec.classList.add("xqac");
...
///14、cookie的添加、删除、获取
	...
	function tuofeng(str){
		let newstr=str;
		let reg=/-/g;
		while(newstr.match(reg)){
			let index=newstr.search(reg);
			let str1=newstr.slice(0,index);
			let str2=newstr.slice(index+1);
